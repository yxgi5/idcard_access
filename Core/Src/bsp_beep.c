...
//#define BEEP_HAVE_POWER   /* 定义此行表示有源蜂鸣器，直接通过GPIO驱动, 无需PWM */
  ...
  #define BEEP_ENABLE() bsp_SetTIMOutPWM(GPIOA, GPIO_PIN_0, TIM2, 1, 1500, 5000);

  /* 禁止蜂鸣器鸣叫 */
  #define BEEP_DISABLE()  bsp_SetTIMOutPWM(GPIOA, GPIO_PIN_0, TIM2, 1, 1500, 0);
#endif
